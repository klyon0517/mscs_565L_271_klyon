...
SelectUIPanel::SelectUIPanel(Vector2i res) :
    ...
    UIPanel(
        ...
        (res.x / 10) * 6, // as wide as 6/10 the resolution
        res.y / 3, // as tall as 1/3 the resolution
...
{
    // Buttons are positioned relative to the
    // top left corner of the UI panel(m_View in UIPanel)
    addButton(
        m_ButtonPadding,
        m_ButtonPadding,
        m_ButtonWidth,
